...
오른쪽의 메뉴에서 스티커나 텍스트의 메뉴를 클릭하면 특정 스티커나 텍스트를 삽입할수 있는 모달을 열수있습니다. 해당 모달은 PC 사이즈와 모바일 사이즈 모두 다르게 작업을 하였습니다.
...
포토카드 위애 나타난 스티커를 클릭하면 스티커를 원하는대로 수정할 수 있는 moveable박스가 생성됩니다. 클릭한 스티커만 moveable박스가 나타나도록 코딩했습니다.
...
포토카드 꾸미기가 완료되었다면, 메뉴를 열어 해당 포토가드를 저장하거나 다운로드 할수 있습니다. 저장 버튼을 누른다면 해당 포토카드를 편집한 날짜와 함께, 꾸민 포토카드는 갤러리에 저장됩니다. Canvas API를 이용하여 다 꾸민 카드의 이미지를 png로 cloudinary에 저장하고, 그 저장한 이미지의 주소를 다시 mongodb의 데이터베이스에 저장하는 방식으로 기능을 구현했습니다.
